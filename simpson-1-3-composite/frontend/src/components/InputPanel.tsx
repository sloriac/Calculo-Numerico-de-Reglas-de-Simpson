...
const PRESETS: Preset[] = [
  ...
  { label: "sin(x)·e^0.3x, n=20", a: 0, b: 4, n: 20, expression: "sin(x) * exp(0.3*x)" },
  { label: "1 / (1 + x²), n=10", a: -1, b: 1, n: 10, expression: "1 / (1 + x**2)" },
];

interface InputPanelProps {
  onSubmit: (a: number, b: number, n: number, expression: string) => void;
  isLoading: boolean;
}
...
export function InputPanel({ onSubmit, isLoading }: InputPanelProps) {
  ...
  function handleSubmit(event: FormEvent) {
    ...
    onSubmit(Number(a), Number(b), Number(n), expression);
  }
  ...
  function applyPreset(preset: Preset) {
    ...
    onSubmit(preset.a, preset.b, preset.n, preset.expression);
  }
  ...
  return (
    <div className="rounded-lg border border-[var(--color-panel-border)] bg-[var(--color-panel)] p-5">
      ...
      <form onSubmit={handleSubmit} className="mt-4 flex flex-col gap-4">
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-sm text-[var(--color-text-dim)]">Subintervalos · n (par)</span>
          <input
            value={n}
