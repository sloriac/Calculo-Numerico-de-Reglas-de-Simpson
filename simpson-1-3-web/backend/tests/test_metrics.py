...
def test_compute_error_against_known_exact_value():
    ...
    assert error.relative_error == pytest.approx(0.0, abs=1e-9)


def test_compute_error_detects_nonzero_discrepancy():
    result = integrate(lambda x: x**4, 0.0, 2.0)
    error = compute_error(result, exact_value=2**5 / 5)
    assert error.absolute_error > 0
    assert error.relative_error > 0


def test_benchmark_reports_three_evaluations_and_positive_time():
    bench = benchmark(lambda x: x**2, 0.0, 2.0, repetitions=50)
    assert bench.evaluation_count == 3
