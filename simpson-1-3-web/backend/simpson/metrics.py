"""Error quantification and performance instrumentation for Simpson 1/3 Simple."""
...
from .core import FunctionOfX, SimpsonSimpleResult, integrate
...
def compute_error(result: SimpsonSimpleResult, exact_value: float) -> ErrorMetrics:
...
def benchmark(function: FunctionOfX, lower_bound: float, upper_bound: float, repetitions: int = 2000) -> BenchmarkResult:
    ...
    A single call completes in sub-microsecond time, below typical timer resolution,
    so repetitions produce a measurement that is actually meaningful.
    """
