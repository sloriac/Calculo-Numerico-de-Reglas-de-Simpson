...
export default function App() {
  ...
  async function runIntegration(a: number, b: number, n: number, expression: string) {
    setIsLoading(true);
    setError(null);
    try {
      const data = await computeIntegration(a, b, n, expression);
      setResult(data);
    } catch (caught) {
  ...
  useEffect(() => {
    runIntegration(0, 2, 6, "x**2 + 2*x + 4");
    fetchExperiments()
      .then(setExperiments)
      ...
      <header className="mb-8 flex items-start justify-between border-b border-[var(--color-panel-border)] pb-5">
        <div>
          <h1 className="font-mono text-2xl font-bold tracking-tight">
            SIMPSON <span className="text-[var(--color-signal)]">3/8</span> COMPUESTA
          </h1>
        ...
        <div className="flex flex-col gap-6">
          ...
          {result && (
            <div className="rounded-lg border border-[var(--color-panel-border)] bg-[var(--color-panel)] p-4">
              <SectionLabel>Vista 3D de cuadratura (paneles encadenados)</SectionLabel>
              <div className="mt-2">
          ...
          {experiments && (
            <div className="rounded-lg border border-[var(--color-panel-border)] bg-[var(--color-panel)] p-4">
              <SectionLabel>Estudio de convergencia empírico (n real)</SectionLabel>
              <div className="mt-2">
