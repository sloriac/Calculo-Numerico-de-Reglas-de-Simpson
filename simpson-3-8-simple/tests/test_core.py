...
def test_step_size_is_a_third_of_interval_width():
    result = integrate(math.sin, 0.0, 3.0)
    assert result.step_size == pytest.approx(1.0)


def test_uses_exactly_four_nodes():
    result = integrate(math.cos, -1.0, 1.0)
    assert result.evaluation_count == 4


def test_node_weights_follow_one_three_three_one_pattern():
    result = integrate(math.exp, 0.0, 1.0)
    assert [node.weight for node in result.nodes] == [1, 3, 3, 1]
